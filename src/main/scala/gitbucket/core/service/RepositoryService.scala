    ...
    getAccountByUserName(newUserName).foreach { _ =>
      (Repositories filter { t =>
        ...
        LockUtil.lock(s"${repository.userName}/${repository.repositoryName}") {
          // Update pull request source-repo columns (no FK constraint; must be explicit).
          PullRequests
            .filter { t =>
          ...
          // Rename the repository row in-place; ON UPDATE CASCADE propagates the new
          // (USER_NAME, REPOSITORY_NAME) to all FK-referencing child tables automatically.
          Repositories
...
//    Activities.filter(_.byRepository(userName, repositoryName)).delete
  ...
  def getRepositoryById(repositoryId: Int)(implicit s: Session): Option[RepositoryInfo] =
    Repositories
