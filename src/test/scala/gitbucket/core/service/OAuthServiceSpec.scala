...
class OAuthServiceSpec extends AnyFunSuite with ServiceSpecBase {
  ...
  test("isLoopbackRedirectUri accepts any port on 127.0.0.1 or localhost over plain http") {
    assert(OAuthService.isLoopbackRedirectUri("http://127.0.0.1:54321/callback"))
    assert(OAuthService.isLoopbackRedirectUri("http://localhost:1/"))
  }

  test("isLoopbackRedirectUri rejects non-loopback, non-http, and lookalike hosts") {
    assert(!OAuthService.isLoopbackRedirectUri("https://127.0.0.1/"))
    assert(!OAuthService.isLoopbackRedirectUri("http://evil.example.com/"))
    assert(!OAuthService.isLoopbackRedirectUri("http://127.0.0.1.evil.com/"))
  }
}
